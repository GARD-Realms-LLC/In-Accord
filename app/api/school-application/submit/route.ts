...
export async function POST(req: Request) {
  ...
  try {
    ...
    const schoolDesignation = String(formData.get("schoolDesignation") ?? formData.get("familyDesignation") ?? "").trim().slice(0, 80);
    ...
    drawTextLines(
      ...
      [
        ...
        `School Designation: ${schoolDesignation}`,
        "",
    ...
    if (r2Client) {
      ...
      const fileName = `${Date.now()}-${safeFileName(profile.userId)}-school-application.pdf`;
      const localSubDir = "school-applications";
    ...
    const nextSchoolCenter = {
      ...currentPreferences.schoolCenter,
      ...
      familyApplicationFiles: [
        {
          name: `${profile.userId}-school-application.pdf`,
          url: pdfUrl,
