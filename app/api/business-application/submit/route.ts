...
export async function POST(req: Request) {
  ...
  try {
    ...
    const businessDesignation = String(formData.get("businessDesignation") ?? "").trim().slice(0, 80);
    const businessSection = String(formData.get("businessSection") ?? "").trim().slice(0, 80);
    ...
    if (!businessDesignation) {
      return NextResponse.json({ error: "Business designation is required." }, { status: 400 });
    }

    if (!businessSection) {
      return NextResponse.json({ error: "Business section is required." }, { status: 400 });
    }
    ...
    drawTextLines(
      coverPage,
      [
        "In-Accord Business Application",
        ...
        `Email: ${email || "Not set"}`,
        `Phone: ${phone || "Not set"}`,
        `Business Role: ${businessDesignation}`,
    ...
    const key = `Client/Applications/${Date.now()}-${safeFileName(profile.userId)}-business-application.pdf`;
    ...
    if (r2Client) {
      ...
      const fileName = `${Date.now()}-${safeFileName(profile.userId)}-business-application.pdf`;
      const localSubDir = "business-applications";
    ...
    const nextBusinessCenter = {
      ...currentPreferences.businessCenter,
      businessDesignation: businessDesignation,
      businessSection,
      businessApplicationStatus: "Submitted",
      businessApplicationSubmittedAt: submittedAtIso,
      businessApplicationFiles: [
        {
          name: `${profile.userId}-business-application.pdf`,
          url: pdfUrl,
    ...
    logPerf("business-application.submit", startedAtMs, `status=200 files=${files.length}`);
    return NextResponse.json({
      ok: true,
      pdfUrl,
      key,
      businessCenter: updated.businessCenter,
    });
  } catch (error) {
    logPerf("business-application.submit", startedAtMs, "status=500");
    console.error("[BUSINESS_APPLICATION_SUBMIT_POST]", error);
