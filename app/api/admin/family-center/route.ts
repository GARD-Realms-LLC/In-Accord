  ...
  try {
    ...
    const result = await db.execute(sql`
      select
        ...
        pref."familyCenterJson" as "familyCenterJson",
    ...
    const rows = ((result as unknown as { rows?: FamilyCenterRow[] }).rows ?? []).filter(
      (row): row is FamilyCenterRow => typeof row?.userId === "string" && row.userId.trim().length > 0
    );
      ...
      .map((row) => {
        ...
        const applicationFiles = Array.isArray(parsed.familyApplicationFiles)
          ? parsed.familyApplicationFiles
              .filter((item): item is FamilyApplicationFile => {
                if (!item || typeof item !== "object") {
                  return false;
                }

                const candidate = item as Partial<FamilyApplicationFile>;
        ...
        const hasFamilyData =
          ...
          Boolean(familyDesignation) ||
          Boolean(applicationStatus) ||
          Boolean(applicationSubmittedAt) ||
          (raw.trim().length > 0 && raw.trim() !== "{}") ||
          /^(FAMILY|FAMILY_ROLE|PARENT|PARENT_ROLE)$/i.test(String(row.role ?? "").trim());
    ...
    const summary = {
      totalFamilyRecords: entries.length,
...
export async function DELETE(req: Request) {
  try {
    ...
    console.error("[ADMIN_FAMILY_CENTER_DELETE]", error);
    return new NextResponse("Internal Error", { status: 500 });
  }
}
...
export async function PATCH(req: Request) {
  try {
    ...
    if (isApproveDecision) {
      nextStatus = `Aproved by: ${approverLabel}`;
    ...
    if (r2Client && nextApplicationFiles.length > 0) {
      ...
      if (sourceKey) {
        ...
        const targetFileName = `${userId}-family-application-${suffix}.pdf`;
    ...
    const nextFamilyCenter = {
      ...
      familyApplicationSubmittedAt:
        current.familyCenter.familyApplicationSubmittedAt || new Date().toISOString(),
    ...
    return NextResponse.json({
      ...
      applicationSubmittedAt: nextFamilyCenter.familyApplicationSubmittedAt,
    });
  } catch (error) {
    console.error("[ADMIN_FAMILY_CENTER_PATCH]", error);
    return new NextResponse("Internal Error", { status: 500 });
  }
}
