...
const hasFamilyCenterAccess = (role: string | null | undefined) => {
...
export async function GET(request: Request) {
  try {
    ...
    if (!hasFamilyCenterAccess(profile.role)) {
    ...
    const rows = (result as unknown as { rows?: FamilyMemberRow[] }).rows ?? [];
    ...
    console.error("[FAMILY_CENTER_MEMBERS_GET]", error);
  ...
  try {
    ...
    if (existingEmail) {
      if (existingEmail.familyParentUserId === profile.id) {
        return new NextResponse("A family account with this email already exists in your Family Center", { status: 409 });
      }
    ...
    if (existingProfileName) {
      if (existingProfileName.familyParentUserId === profile.id) {
        return new NextResponse("A family account with this profile name already exists in your Family Center", { status: 409 });
      }
    ...
    console.error("[FAMILY_CENTER_MEMBERS_POST]", error);
    return new NextResponse("Internal Error", { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    const member = (result as unknown as { rows?: FamilyMemberRow[] }).rows?.[0];

    if (!member) {
      return new NextResponse("Family member account not found", { status: 404 });
    }

    if (member.familyParentUserId !== profile.id && !isInAccordAdministrator(profile.role)) {
      return new NextResponse("Only the creating family account can convert this member", { status: 403 });
    }
    ...
    console.error("[FAMILY_CENTER_MEMBERS_PATCH]", error);
    return new NextResponse("Internal Error", { status: 500 });
  }
}
